<template>
  <div class="flex flex-col flex-1 items-center">
    ...
    <div
      ...
      <p>
        Feels like
        {{ Math.round(weatherData.current.feels_like - 273.15) }}&deg;
      </p>
      ...
      <div class="mx-8 text-white">
        ...
        >
          ...
          <div class="flex gap-2 flex-1 justify-end">
            <p>H: {{ Math.round(day.temp.max - 273.15 ) }}&deg;</p>
            <p>L: {{ Math.round(day.temp.min - 273.15 ) }}&deg;</p>
          </div>
        </div>
      </div>
    </div>

    <!-- Remove from localStorage button -->
...
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/3.0/onecall?lat=${route.query.lat}&lon=${route.query.lng}&appid=f1bf7ae55e2f27a575b60e0159060180`
    );
